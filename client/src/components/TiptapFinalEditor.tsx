...
interface TiptapFinalEditorProps {
  content: string
  onChange: (content: string) => void
  placeholder?: string
}

export function TiptapFinalEditor({ content, onChange, placeholder }: TiptapFinalEditorProps) {
            ...
            if (htmlContent.includes('<table')) {
              ...
              if (table) {
                ...
                setTimeout(() => {
                  setClipboardStatus({
                    type: 'table',
                    message: `엑셀 표가 편집 가능한 표로 변환되었습니다! (${rows.length}행 × ${maxCols}열)`,
                    variant: 'success'
                  })
  ...
  const insertSampleTable = () => {
    if (editor) {
      ...
      setTimeout(() => {
        const sampleData = [
          ['구분', '항목명', '수량', '비고'],
          ['하드웨어', 'CPU 업그레이드', '2개', '긴급'],
          ['소프트웨어', '보안 패치', '5개', '완료'],
          ['네트워크', '방화벽 설정', '1개', '진행중']
        ]
        
        // Focus on the first cell and add sample data
        const firstCell = document.querySelector('.ProseMirror table td, .ProseMirror table th')
        if (firstCell) {
          (firstCell as HTMLElement).textContent = sampleData[0][0]
        }
  ...
  if (!editor) {
    return <div className="border rounded-lg p-4">Loading Tiptap editor...</div>
  }

  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">Tiptap 고급 에디터</h3>
